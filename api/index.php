...
function checkMaintenanceMode() {
    global $db;
    $result = $db->query("SELECT title, body, mode FROM maintenance WHERE id=1");
    $row = $result->fetch_assoc();
    $sc = generateRandomSC();
    $response = [
        "result" => "success",
        "sc" => $sc,
        "maintenancemode" => $row['mode'] ?? 'off',
        "message" => $row['title'] ?? '',
        "footercontent" => $row['body'] ?? ''
    ];
...
function getadvertisement() {
    global $db;
    $result = $db->query("SELECT title, text FROM advertisement WHERE id=1");
    $row = $result->fetch_assoc();
...
function addDevice($data) {
    ...
    $stmt->bind_param("sss", $deviceid, $deviceusername, $date);
    $stmt->execute();
    $sc = generateRandomSC();
    $response = [
        "result" => "success",
...
function addReport($data) {
    ...
    $stmt->bind_param("sssssssssi", 
        $data['username'], $data['macaddress'], $data['section'], $data['section_category'],
        $data['report_title'], $data['report_sub_title'], $data['report_cases'], 
        $data['report_custom_message'], $data['stream_name'], $data['stream_id']
    );
    if (!$stmt->execute()) {
        logError($db->error);
...
function addClientFeedback($data) {
    ...
    $stmt->bind_param("sss", $data['username'], $data['macaddress'], $data['feedback']);
    $stmt->execute();
    return json_encode(["result" => "success", "message" => "Feedback sent successfully!"]);
}
...
function getAnnouncement($data) {
    ...
    foreach ($announcements as $announcement) {
        ...
        $stmt->bind_param("is", $announcement['id'], $data['deviceid']);
        $stmt->execute();
        $res = $stmt->get_result();
        $seen = $res->num_rows > 0 ? 1 : 0;
...
function getEnhancedAnnouncement($data) {
    ...
    $rewardedAds = ['add_status' => '1', 'add_viewable_rate' => '2', 'add_type' => 'rewarded', 'message' => 'advertisement data', 'data' => []];
    $dashboardAds = ['add_status' => '1', 'message' => 'advertisement data', 'data' => []];
    return json_encode(["result" => "success", "sc" => $sc, "rewarded" => $rewardedAds, "dashboard" => $dashboardAds]);
}

function processAdData() {
    global $db;
    $tmdbResult = $db->query("SELECT tmdb_api_enabled FROM settings WHERE id=1");
    $tmdbRow = $tmdbResult->fetch_assoc();
    $tmdbApiEnabled = $tmdbRow['tmdb_api_enabled'] ?? 0;

    $rewardedResult = $db->query("SELECT text FROM ads WHERE id=1");
    $rewardedRow = $rewardedResult->fetch_assoc();
    if (!$rewardedRow) {
        return json_encode(["result" => "error", "message" => "Failed to retrieve rewarded ads data"]);
    }

    $rewardedAds = [[
        "id" => rand(10, 99), "title" => "Bet3", "type" => "message", "pages" => "dashboard",
        "position" => "top", "schedule_type" => "alltime", "number" => "", "redirect_link" => "",
        "custom_recc" => "", "text" => $rewardedRow['text'], "images" => []
    ]];

    $ads2TextResult = $db->query("SELECT text FROM ads2 WHERE id=1");
    $ads2TextRow = $ads2TextResult->fetch_assoc();
    ...
    $dashboardAds = [[
        "id" => rand(1000, 9999), "title" => "Bet3", "type" => "image", "pages" => "dashboard",
        "position" => "top", "schedule_type" => "alltime", "number" => "", "redirect_link" => "",
        "custom_recc" => "", "images" => $images, "text" => $ads2TextRow ? $ads2TextRow['text'] : ''
    ]];

    return json_encode([
        "result" => "success",
        "sc" => "3e7a97f0e75fd54f5c88e8bf144edcc5",
        "rewarded" => ["add_status" => "1", "add_viewable_rate" => "5", "add_type" => "rewarded", "message" => "ads data", "totalrecords" => count($rewardedAds), "timeinterval" => "", "data" => $rewardedAds],
        "dashboard" => ["add_status" => "1", "message" => "ads data", "totalrecords" => count($dashboardAds), "timeinterval" => "", "data" => $dashboardAds]
    ]);
}
...
function fetchTMDBData() {
    ...
    for ($i = 0; $i < $maxCount; $i++) {
        if ($i < $numMovies) {
            ...
            $combined_data[] = ["id" => $movie['id'], "artWork" => 'https://image.tmdb.org/t/p/original' . $movie['poster_path'], "title" => $movie['title']];
        }
        if ($i < $numShows) {
            $show = $shows_data['results'][$i];
            $combined_data[] = ["id" => $show['id'], "artWork" => 'https://image.tmdb.org/t/p/original' . $show['poster_path'], "title" => $show['name']];
        }
    }
